...
`apis/valmer_control_plane/app.py` exports the FastAPI application. Business logic is reusable
...
1. sync `valmer-connectors` so the platform indexes `apis/valmer_control_plane/app.py` as a FastAPI
...
```bash
.venv/bin/ruff check src/valmer_connectors/control_plane apis/valmer_control_plane \
